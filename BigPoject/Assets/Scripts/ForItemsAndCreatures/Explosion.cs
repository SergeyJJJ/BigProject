...
namespace ForItemsAndCreatures
{
    public class Explosion : MonoBehaviour
    {
        ...
        public void Explode()
        {
            Collider2D[] affectedColliders = Physics2D.OverlapCircleAll(_explosionCenterPoint.position, _explosionRadius, _affectedByExplosion);
            
            foreach (Collider2D collider in affectedColliders)
            {
                Rigidbody2D rigidbody = collider.GetComponent<Rigidbody2D>();
                Vector2 forceDirection = (collider.transform.position - _explosionCenterPoint.position).normalized;
